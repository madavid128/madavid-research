...
- `make_mad_icons.py` — generate static icons + animated GIF variants
- `tools/gallery_from_csv.py` — CSV-first gallery pipeline (generate `_data/pictures.yaml` / `_data/art.yaml`)
- `tools/gallery_subtitles.py` — spreadsheet workflow for `subtitle` + `alt`
- `tools/generate_third_party_notices.py` — generate/update `THIRD_PARTY_NOTICES.md`
...
- `tools/watermark_galleries.py` — build `images/wm/` from `images/originals/` (watermarks + conversions)
